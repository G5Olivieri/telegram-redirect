...
static jlong TelegramNativeClient_createNativeClient(JNIEnv *env, jclass clazz) {
  return static_cast<jlong>(reinterpret_cast<std::uintptr_t>(new td::Client()));
}

static void TelegramNativeClient_nativeClientSend(JNIEnv *env, jclass clazz, jlong client_id, jlong id, jobject function) {
  get_client(client_id)->send({static_cast<std::uint64_t>(id), fetch_function(env, function)});
}

static jint TelegramNativeClient_nativeClientReceive(JNIEnv *env, jclass clazz, jlong client_id, jlongArray ids, jobjectArray events,
                                       jdouble timeout) {
...
static jobject TelegramNativeClient_nativeClientExecute(JNIEnv *env, jclass clazz, jobject function) {
  jobject result;
  td::Client::execute({0, fetch_function(env, function)}).object->store(env, result);
  return result;
}

static void TelegramNativeClient_destroyNativeClient(JNIEnv *env, jclass clazz, jlong client_id) {
...
static jint register_native(JavaVM *vm) {
  ...
  auto client_class = td::jni::get_jclass(env, PACKAGE_NAME "/TelegramNativeClient");
...
#define TD_FUNCTION "L" PACKAGE_NAME "/TdApi$Function;"
  register_method(client_class, "createNativeClient", "()J", TelegramNativeClient_createNativeClient);
  register_method(client_class, "nativeClientSend", "(JJ" TD_FUNCTION ")V", TelegramNativeClient_nativeClientSend);
  register_method(client_class, "nativeClientReceive", "(J[J[" TD_OBJECT "D)I", TelegramNativeClient_nativeClientReceive);
  register_method(client_class, "nativeClientExecute", "(" TD_FUNCTION ")" TD_OBJECT, TelegramNativeClient_nativeClientExecute);
  register_method(client_class, "destroyNativeClient", "(J)V", TelegramNativeClient_destroyNativeClient);
